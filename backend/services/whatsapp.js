 ...
 * @param {object} visit        - must include changedBy for correct signature
 * @param {number} balanceDue
 * @param {object} options      - { includeTreatment, includeInstructions, includeDues }
 */
async function sendVisitMessage(patient, visit, balanceDue, options = {}) {
  const { includeTreatment = false, includeInstructions = true, includeDues = false } = options;
  ...
  // Build message based on toggles
  let message = `Hello ${patient.name}! 👋\n\nThank you for visiting us today.\n\n`;

  if (includeTreatment && visit.treatmentDoneToday) {
    message += `🦷 *Treatment Done Today:*\n${visit.treatmentDoneToday}\n\n`;
  }

  if (includeInstructions && visit.medicinesInstructions) {
  ...
  // Use the logged-in user who added the visit, not the assigned doctor
